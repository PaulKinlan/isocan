...
**187 capabilities across six sources — 124 tooled, 34 missing, 29 excluded with a reason.**
...
| Voice harness (`packages/cli/src/voice-harness.ts`, `LIVE_TOOLS`) | 42 | 42 | 0 | 0 |
...
## 6. Voice harness tool declarations — `packages/cli/src/voice-harness.ts` (42 rows)
...
| Capability | Tool | Status | Reason |
|---|---|---|---|
| Add a note, a card, a link or an image | `add_item` | **tooled** |  |
| Rename an item | `rename_item` | **tooled** |  |
| Change an item's text, title or description | `update_item` | **tooled** |  |
| Move an item to the trash | `delete_item` | **tooled** |  |
| Bring an item back from the trash | `restore_item` | **tooled** |  |
| Move an item | `move_item` | **tooled** |  |
| Resize an item | `resize_item` | **tooled** |  |
| Say something in the Chat | `say` | **tooled** |  |
| Ask a question in the Chat and wait for an answer | `ask` | **tooled** |  |
| Leave a comment on an item | `comment_on_item` | **tooled** |  |
| Keep an item's text as a new version | `item_add_version` | **tooled** |  |
| Start a thread on an item | `thread_create` | **tooled** |  |
| Pin a thread to a point on the canvas | `thread_set_anchor` | **tooled** |  |
| Make a thread the canvas's main Chat | `thread_set_main` | **tooled** |  |
| Delete a thread and its comments | `thread_delete` | **tooled** |  |
| Edit a comment | `comment_update` | **tooled** |  |
| Post in the Chat without waiting for an answer | `notify` | **tooled** |  |
| Change the colour of its own presence | `actor_set_color` | **tooled** |  |
| Change the emoji on its own presence | `actor_set_mark` | **tooled** |  |
| Fold another of this machine's identities into itself | `actor_join` | **tooled** |  |
| Enrol another agent so it can be summoned by name | `agent_enroll` | **tooled** |  |
| Withdraw an enrolled agent | `agent_withdraw` | **tooled** |  |
| Draw ink on the canvas | `drawing_add` | **tooled** |  |
| React to an item with an emoji | `item_react` | **tooled** |  |
| Find items by what they say | `find_items` | **tooled** |  |
| Move several items at once | `items_move` | **tooled** |  |
| Move several items to the trash | `items_delete` | **tooled** |  |
| Bring several items back | `items_restore` | **tooled** |  |
| Switch an item to another version | `item_set_current_version` | **tooled** |  |
| Bring an item into somebody's view | `viewport_focus` | **tooled** |  |
| Pan the view | `viewport_pan` | **tooled** |  |
| Select items | `selection_set` | **tooled** |  |
| Clear the selection | `selection_clear` | **tooled** |  |
| Read the canvas | `read_canvas` | **tooled** |  |
| Read one item, and its versions | `read_item` | **tooled** |  |
| Read threads and comments | `read_threads` | **tooled** |  |
| See who is live on the canvas | `read_presence` | **tooled** |  |
| Remember a fact for a later session | `remember` | **tooled** |  |
| List the folder you granted | `list_dir` | **tooled** |  |
| Read one file from the folder you granted | `read_file` | **tooled** |  |
| Read one stored memory | `read_memory` | **tooled** |  |
| Search its memories | `search_memory` | **tooled** |  |
