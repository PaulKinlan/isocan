...
| **`pandoc-wasm`** | Universal Markdown / DOCX / LaTeX converter | **Binary size is 80–120 MB**. Haskell GHC Wasm backend embeds the complete Haskell runtime, garbage collector, and gigantic parser tables. Startup latency exceeds 2 seconds. | **REJECTED as a portable tool: massive binary size and slow cold-start. Redirect** — universal document conversion belongs to a daemon-side pandoc binary behind the compute host supervisor (as designed in `54k.3`), or a lighter conversion path for narrower formats (e.g. unified/remark for Markdown, mammoth for DOCX). |
