...
describe("a design competition, from the terminal", () => {
  it("plays from the line-up to standings, and says every refusal", async () => {
    ...
    // Scene 1 — the arena.
    const beforeArena = await daemon.engine.getSnapshot(C);
    ...
    const arenaSnapshot = await daemon.engine.getSnapshot(C);
    expect(arenaSnapshot.lastSeq).toBe(beforeArena.lastSeq + 1);
    expect((await daemon.engine.getLog(C)).at(-1)!.envelope.op).toMatchObject({ type: "group.change", action: { kind: "apply", change: { intent: "create" } } });
    await ok(["undo", "--canvas", C]);
    expect((await daemon.engine.getSnapshot(C)).canvas.items).toEqual(beforeArena.canvas.items);
    await ok(["redo", "--canvas", C]);
    const stableItems = (items: typeof arenaSnapshot.canvas.items) => Object.fromEntries(Object.entries(items).map(([id, { updatedAt: _at, updatedBy: _by, ...record }]) => [id, record]));
    const restored = await daemon.engine.getSnapshot(C);
    expect(stableItems(restored.canvas.items)).toEqual(stableItems(arenaSnapshot.canvas.items));
    const original = restored.canvas.items[bout]!;
    const invalid = preparedGroupCreation(C, [
      { id: "itm_acme_new_frame", title: "Acme rejected", version: { ...original.versions[0]!, id: "ver_acme_rejected_frame" }, properties: { kind: "group" }, box: { x: 20000, y: 0, width: 500, height: 500 } },
      { id: "itm_acme_new_child", title: "Acme rejected child", version: { ...original.versions[0]!, id: "ver_acme_rejected_child" }, containerId: "itm_missing", box: { x: 20024, y: 80, width: 200, height: 200 } },
    ], { x: 20000, y: 0 });
    const identity = JSON.parse(await fs.readFile(path.join(home, "identity.json"), "utf8"));
    const auth = Object.values(identity.auth)[0] as { badgeId: string; secret: string };
    const rejected = await fetch(`${base}/api/ops`, { method: "POST", headers: { "content-type": "application/json", [CLIENT_FEATURES_HEADER]: CANVAS_GROUPS_FEATURE, authorization: `Bearer ${formatBadgeToken(auth.badgeId, auth.secret)}` }, body: JSON.stringify({ canvasId: C, actor: priya, op: invalid }) });
    expect(rejected.status).toBe(400);
    expect(await daemon.engine.getSnapshot(C)).toEqual(restored);
