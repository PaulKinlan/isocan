...
async function layArena(
  ...
  const plan = arenaPlan({
    canvasId, groupMode: snapshot.project.groupMode ?? "legacy",
    ...
    .action(
      run(async (ref: string | undefined, _opts: unknown, cmd: Command) => {
        ...
        await sendOp(ctx, canvas.id, {
          ...
          itemId: target.id,
          version: { id: newVersionId(), blobHash: v.blobHash, size: v.size, mimeType: v.mimeType, filename: v.filename, ...(v.visual ? { visual: v.visual } : {}) },
        });
