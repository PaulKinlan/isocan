...
export function useTouchNavigation(step: (direction: Direction) => void, plan?: () => void, thirds = false) {
  ...
  if (!state.current) state.current = touchNavigation((d) => action.current.step(d), () => action.current.plan?.(), thirds);
  return {
    onPointerDownCapture: (e: React.PointerEvent) => state.current!.down(e),
    onPointerMoveCapture: (e: React.PointerEvent) => state.current!.move(e),
    onPointerUpCapture: (e: React.PointerEvent) => state.current!.up(e.pointerId, window.innerWidth),
    onPointerCancelCapture: () => state.current!.cancel(),
    onClickCapture: (e: React.MouseEvent) => { if (state.current!.consumeClick()) { e.preventDefault(); e.stopPropagation(); } },
  };
}
