...
/**
 * **The page's own dev server, and its own build.**
 *
 ...
 * this package the reasoning inverts: the page IS the build, and there is no
 * app to protect.
 *
 * `base: "/"` and absolute module paths (`/src/main.ts`) stay as they were, and
 * that is what makes the serve path one line over in `packages/web`: the app's
 * server redirects `/voice` here rather than proxying under a prefix, so the
 * page's origin is always the server that serves it. Same-origin is not
...
export default defineConfig(({ command }) => ({
  ...
  server: {
    port: 5200,
    // A lane that silently lands on another port is a lane that measures the
    // wrong server; two of those cost a night (see the extraction plan).
    strictPort: true,
    proxy: {
      /**
