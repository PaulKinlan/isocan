...
export interface LiveSession {
   ...
   * (isocan-xsh.8.6). Sent once per pause — a mute past a second under
   * automatic VAD — and the next frame begins a new segment, so unimute needs no
   * matching "start". Declared here because this interface is what callers see:
   * the method existed on the implementation before this interface was written
   * down, and the gate that refused the push is what noticed
   * (isocan-1j1).
   */
  endAudioStream(): void;
