 ...
 */
export const SHELVED_PROP = "shelved";

/** When this canvas was put away, or null if it is in the list. */
export function shelvedAt(canvas: { properties?: Record<string, string> }): string | null {
  return canvas.properties?.[SHELVED_PROP] ?? null;
}

/** Is it off the list? */
export function isShelved(canvas: { properties?: Record<string, string> }): boolean {
 ...
 */
export function shelvePatch(when: string): MetaPatch {
...
export function unshelvePatch(): MetaPatch {
