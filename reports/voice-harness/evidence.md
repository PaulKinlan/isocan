...
Run 2026-09-11T23:44:17.290Z in 3.804s.

- browser: HeadlessChrome 152.0.0.0
- connected to: canvas “Voice evidence” (prj_voice), daemon http://127.0.0.1:42123, home no home configured — the daemon's default
- agent: Voice (usr_mtQfjEwyFL), NOT enrolled
...
- bars lit immediately after the shutter closed: 6/28 (the level meter has a slow release on purpose; the held-peak marker is what survives a quiet moment)
...
## Steps
- harness: `isocan voice` listening at http://127.0.0.1:7948/ (daemon on 42123, home isocan-voice-evidence-CD0hVC)
- connected to: canvas “Voice evidence” prj_voice · daemon http://127.0.0.1:42123 · home no home configured — the daemon's default · agent Voice usr_mtQfjEwyFL (not enrolled) · audio no provider models/gemini-3.1-flash-live-preview, no key
...
- stop: listening stopped — 2.0s of audio | live session open — the harness holds the key and the socket
- utterance: “retitle the checkout screen to Checkout v2”
...
- canvas: last op item.update by Voice (usr_mtQfjEwyFL), the enrolled agent
